...
${\color{lightgreen}\text{Bool-M-CBE}}$: a variant of Sym-M-CBE designed for binary concept and classification tasks. Each expert is a Boolean logic rule over the predicted concepts. The allowed operators are defined at the top of [src/models/bool_m_cbe.py](src/models/bool_m_cbe.py) via the `boolean_binary_operators` and `boolean_unary_operators` variables. As with Sym-M-CBE, PySR parameters can be configured via the `pysr_params` field in [conf/model/bool_m_cbe.yaml](conf/model/bool_m_cbe.yaml).
