...
class Trainer:
    ...
    def allow_symbolic(self, train_dataloader, val_dataloader):
        ...
        # For Symbolic M-CBE, collect data for symbolic fine-tuning
        if model_name == 'sym_m_cbe':
        ...
        elif model_name in ['mlp_m_cbe', 'lin_m_cbe']:
        ...
        if model_name == 'sym_m_cbe':
            # Higher LR for Sym-M-CBE as there only few parameters in the predictor
    ...
    def substitute_symbolic_equations(self, equations, train_dataloader, val_dataloader):
        ...
        if model_name != 'sym_m_cbe':
            raise ValueError(f"substitute_symbolic_equations only supports 'sym_m_cbe'. Got: {model_name}")
